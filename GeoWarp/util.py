...
def compute_features(geoloc_dataset, model, global_features_dim, num_workers=4,
    ...
    Parameters
    ----------
    geoloc_dataset : dataset_geoloc.GeolocDataset,  which contains the images (queries and database).
    ...
    model = model.eval()
    with torch.no_grad():
        database_features = np.empty((len(geoloc_dataset), global_features_dim), dtype="float32")
    ...
    query_features = database_features[geoloc_dataset.database_num:]
    database_features = database_features[:geoloc_dataset.database_num]
