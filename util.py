...
def resume_train(args: Namespace, output_folder: str, model: torch.nn.Module,
                 model_optimizer: Type[torch.optim.Optimizer], lr_scheduler: torch.optim.lr_scheduler, classifiers: List[MarginCosineProduct],
                 classifiers_optimizers: List[Type[torch.optim.Optimizer]], classifiers_lr_schedulers: List[torch.optim.lr_scheduler.CosineAnnealingLR]):
    ...
    for c, sd in zip(classifiers_lr_schedulers, checkpoint["classifiers_lr_schedulers_state_dict"]):
        c.load_state_dict(sd)
    ...
    return model, model_optimizer, lr_scheduler, classifiers, classifiers_optimizers, classifiers_lr_schedulers, best_val_recall1, start_epoch_num
